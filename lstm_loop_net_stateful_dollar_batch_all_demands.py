...
def append_prev_demand(vars_ds, demand_ds):
    """ Crea un nuevo dataset que contenga los valores de las variables de entrada y una columna extra que representa la demanda del dia anterior """
...
# Agrego al tensor de demanda de entrada y salida los valores de demanda neta
in_demand = demand_ds[:, 0]
out_demand = demand_ds[:, 1]
__net_demand = in_demand - out_demand
__net_demand.resize([len(__net_demand), 1])
demand_ds = numpy.hstack([demand_ds , __net_demand])

# GUARDO los valores originales de demanda para calcular el error mas adelante
DEMAND = demand_ds.copy()

CAT_COUNT = 50
demand_ds, IN_DEMAND_CATEGORIES = categorizer.categorize_real_w_equal_frames(demand_ds, CAT_COUNT, cat_col=0)
demand_ds, OUT_DEMAND_CATEGORIES = categorizer.categorize_real_w_equal_frames(demand_ds, CAT_COUNT, cat_col=1)
demand_ds, NET_DEMAND_CATEGORIES = categorizer.categorize_real_w_equal_frames(demand_ds, CAT_COUNT, cat_col=2)

# Agrego el valor del dolar al dataset de variables
...
DOW_COL, DOM_COL, MONTH_COL, HOLIDAY_COL, DOLLAR_COL , IN_DEMAND_COL , OUT_DEMAND_COL , NET_DEMAND_COL = range(vds_col_count)
...
for i in range(0, vds_size - seq_length + 1, 1):
    ...
    seq_out = demand_ds[i + seq_length - 1]
    dataX.append(seq_in)
