...
# PRIMERO HACEMOS UNA PREDICCION CON DESCARTE DEL PERIODO DE ENTRENAMIENTO...
train_predicted = []
pattern = dataX[train_lower_limit]

last_idx = train_size - 1
for i in range(train_size):
    ...
    # verificar si debe ser 'batch_size-1' o '0'
    predicted_category = numpy.argmax(prediction[0])
    train_predicted.append(predicted_category)
    if i == last_idx:
        break
    # luego obtenemos los datos de entrada de la prediccion N+1 ...
    next_pattern = dataX[train_lower_limit + i + 1].copy()
...
# LUEGO PREDECIMOS EL PERIODO DE PRUEBAS...
predicted = []
